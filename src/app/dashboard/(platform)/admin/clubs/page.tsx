...
export default async function ClubsPage({ searchParams }: PageProps) {
	const { search, sortBy, sortOrder, page, clubId } = await searchParams;
	...
	const selectedClub = clubId ? await prisma.club.findUnique({
		where: { id: clubId },
	}) : null;
	...
	return (
		<>
			<div>
				<h3 className="text-lg font-semibold">Svi klubovi</h3>
			</div>
